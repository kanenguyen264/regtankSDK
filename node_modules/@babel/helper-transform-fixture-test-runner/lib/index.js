...
function createContext() {
  const context = _vm.default.createContext(Object.assign({}, helpers, {
    process: process,
  ...
  runModuleInTestContext("@babel/polyfill/dist/polyfill.min.js", __filename, context, moduleCache);
  runCacheableScriptInTestContext(_path.default.join(__dirname, "babel-helpers-in-memory.js"), babel.buildExternalHelpers, context, moduleCache);
  return context;
}
...
function runCacheableScriptInTestContext(filename, srcFn, context, moduleCache) {
  ...
  const script = new _vm.default.Script(cached.code, {
    filename,
    displayErrors: true,
    lineOffset: -1,
...
function runModuleInTestContext(id, relativeFilename, context, moduleCache) {
  ...
  const module = runCacheableScriptInTestContext(filename, () => _fs.default.readFileSync(filename, "utf8"), context, moduleCache);
...
function runCodeInTestContext(code, opts, context = sharedTestContext) {
  ...
  try {
    ...
    return _vm.default.runInContext(src, context, {
      filename,
      displayErrors: true,
      lineOffset: -1
